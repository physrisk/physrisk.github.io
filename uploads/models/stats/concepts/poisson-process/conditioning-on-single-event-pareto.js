...
const EXPERIMENTS_PER_UPDATE = 1000;
const INTERVAL = 100;
let continue_flag = false;

let pareto_power = 2;
const MIN_PARETO = 0.025;
let observation_period = 1;
let failure_count = 0;
let event_count = 0;

const N_HISTOGRAM_POINTS = 20;
...
function conduct_single_experiment() {
    let first_event_time = MIN_PARETO * rng.pareto(pareto_power);
    let second_event_time = MIN_PARETO * rng.pareto(pareto_power);
    if (
...
start_btn.addEventListener("click", () => {
    ...
    pareto_power = my_parse_float(
        document.getElementById("pareto_power").value
    );
